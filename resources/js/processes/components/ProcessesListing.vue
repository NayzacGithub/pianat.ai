<template>
  <div class="data-table">
    <data-loading :for="/\/processes\?page/" v-show="shouldShowLoader" :empty="$t('No Data Available')"
      :empty-desc="$t('')" empty-icon="noData" />
    <div v-show="!shouldShowLoader" class="" data-cy="processes-table">
      <vuetable :dataManager="dataManager" :sortOrder="sortOrder" :css="css" :api-mode="false"
        @vuetable:pagination-data="onPaginationData" :fields="fields" :data="data" data-path="data"
        pagination-path="meta" :noDataTemplate="$t('No Data Available')">
        <template slot="name" slot-scope="props">
          <i tabindex="0" v-b-tooltip :title="props.rowData.warningMessages.join(' ')"
            class="text-warning fa fa-exclamation-triangle"
            :class="{'invisible': props.rowData.warningMessages.length == 0}">
          </i>
          <i tabindex="0" v-if="props.rowData.status == 'ACTIVE' || props.rowData.status == 'INACTIVE'" v-b-tooltip
            :title="props.rowData.status" class="mr-2"
        ...
        <template slot="owner" slot-scope="props">
          <avatar-image class="d-inline-flex pull-left align-items-center" size="25" :input-data="props.rowData.user"
            hide-name="true"></avatar-image>
        </template>

        <template slot="actions" slot-scope="props">
          <div class="actions">
            <div class="popout">
              <b-btn variant="link" @click="onAction('unpause-start-timer', props.rowData, props.rowIndex)"
                v-b-tooltip.hover :title="$t('Unpause Start Timer Events')"
                v-if="props.rowData.has_timer_start_events && props.rowData.pause_timer_start"
                v-uni-aria-describedby="props.rowData.id.toString()">
                <i class="fas fa-play fa-lg fa-fw"></i>
              </b-btn>
              <b-btn variant="link" @click="onAction('pause-start-timer', props.rowData, props.rowIndex)"
                v-b-tooltip.hover :title="$t('Pause Start Timer Events')"
                v-if="props.rowData.has_timer_start_events && !props.rowData.pause_timer_start"
                v-uni-aria-describedby="props.rowData.id.toString()">
                <i class="fas fa-pause fa-lg fa-fw"></i>
              </b-btn>
              <b-btn variant="link" @click="onAction('edit-designer', props.rowData, props.rowIndex)" v-b-tooltip.hover
                :title="$t('Edit')"
                v-if="permission.includes('edit-processes') && (props.rowData.status === 'ACTIVE' || props.rowData.status === 'INACTIVE')"
                v-uni-aria-describedby="props.rowData.id.toString()">
                <i class="fas fa-pen-square fa-lg fa-fw"></i>
              </b-btn>
              <b-btn variant="link" @click="onAction('edit-item', props.rowData, props.rowIndex)" v-b-tooltip.hover
                :title="$t('Configure')"
                v-if="permission.includes('edit-processes') && (props.rowData.status === 'ACTIVE' || props.rowData.status === 'INACTIVE')"
                v-uni-aria-describedby="props.rowData.id.toString()">
                <i class="fas fa-cog fa-lg fa-fw"></i>
              </b-btn>
              <b-btn variant="link" @click="onAction('view-documentation', props.rowData, props.rowIndex)"
                v-b-tooltip.hover :title="$t('View Documentation')"
                v-if="permission.includes('view-processes') && isDocumenterInstalled"
                v-uni-aria-describedby="props.rowData.id.toString()">
                <i class="fas fa-map-signs fa-lg fa-fw"></i>
              </b-btn>
              <b-btn variant="link" @click="onAction('export-item', props.rowData, props.rowIndex)" v-b-tooltip.hover
                :title="$t('Export')" v-if="permission.includes('export-processes')"
                v-uni-aria-describedby="props.rowData.id.toString()">
                <i class="fas fa-file-export fa-lg fa-fw"></i>
              </b-btn>
              <b-btn variant="link" @click="onAction('remove-item', props.rowData, props.rowIndex)" v-b-tooltip.hover
                :title="$t('Archive')"
              ...
              <b-btn variant="link" @click="onAction('restore-item', props.rowData, props.rowIndex)" v-b-tooltip.hover
                :title="$t('Restore')"
      ...
      <pagination :single="$t('Process')" :plural="$t('Processes')" :perPageSelectEnabled="true"
        @changePerPage="changePerPage" @vuetable-pagination:change-page="onPageChange" ref="pagination"></pagination>
    </div>
  </div>
</template>
